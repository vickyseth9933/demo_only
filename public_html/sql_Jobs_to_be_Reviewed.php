...
if($searchValue != ''){
   $searchQuery = " AND (cb_order_new.order_no like '%".$searchValue."%' OR 
        CONCAT(cb_user.first_name, ' ' , cb_user.last_name) like '%".$searchValue."%' OR cb_order_new.mat LIKE '%".$searchValue."%' OR cb_order_new.description LIKE '%".$searchValue."%' OR cb_order_new.resp_group LIKE '%".$searchValue."%' OR cb_order_new.recommendation LIKE '%".$searchValue."%')";
}
$searchQuery2 = " ";
if($jobstatus != ''){
...
if($sortField != ''){
   ...
   $sortFieldQuery = "ORDER BY cb_front_cover.resp_group,cb_order_new.RESPONSIBLE_GROUP  $sortOrder";
	}
...
WHERE cb_order_new.user_id =$userid AND cb_order_new.order_stage!='7' AND cb_order_new.reject_status='0' AND cb_order_new.remove_status='0' $searchQuery $searchQuery2 $sortFieldQuery LIMIT  $limit OFFSET $start_fromJobsforApproval";
...
LEFT JOIN cb_front_cover ON(cb_front_cover.order_id=cb_order_new.order_no)
LEFT JOIN cb_project_details ON(cb_project_details.order_id=cb_order_new.order_no)
WHERE cb_order_new.user_id =$userid AND cb_order_new.order_stage!='7' AND cb_order_new.reject_status='0' AND cb_order_new.remove_status='0' $searchQuery $searchQuery2 $sortFieldQuery ";  
