...
if($sortField != ''){
   ...
   $sortFieldQuery = "ORDER BY cb_order.status  $sortOrder";
	...
	else if($sortField=='DateofSubmission'){
   $sortFieldQuery = "ORDER BY cb_front_cover.dateofreview  $sortOrder";
	}
...
$roleID = explode(',',$role_id);
 // $sql = "SELECT * FROM cb_order_new WHERE user_id =$userid AND order_stage!='7' AND reject_status='0'";
     ...
     $sql = "SELECT completed_jobs.complete_status,cb_order.status,cb_order_new.order_no,cb_order_new.description as cb_desc,cb_order_new.recommendation,cb_order_new.user_id,cb_order_new.order_stage,CONCAT(cb_user.first_name, ' ' , cb_user.last_name)  as name,cb_front_cover.order_description as description,cb_front_cover.resp_group,cb_order_new.date_of_submission as dateofreview,cb_order_new.mat as cb_mat,
...
WHERE cb_order_new.order_stage=5 AND  send_job_approval='1' $searchQuery $searchQuery2 $sortFieldQuery LIMIT $start_fromJobsforApproval, $limit";
...
LEFT JOIN cb_front_cover ON(cb_front_cover.order_id=cb_order_new.order_no)
LEFT JOIN cb_project_details ON(cb_project_details.order_id=cb_order_new.order_no)
WHERE cb_order_new.order_stage=5 AND  send_job_approval='1' $searchQuery $searchQuery2 $sortFieldQuery";  
                                        ...
                                        <div class="table-responsive">
                                            <table class="table table-bordered table-hover" id="dash_datatable">
                                                <thead class="thead-default thead-lg">
                                                 ...
                                                 <th onclick="displayRecordsbysort('<?php echo $limit;  ?>', '<?php echo 1; ?>','<?php echo $searchValue; ?>','<?php echo $jobstatus; ?>','Assignto')"><b>Assign To</b> <span  style="top: 5px;right:2px;"><?php if($sortField=='Assignto'){ ?><span class="<?php if($sortOrder=='DESC'){ echo 'text-white';} ?>" >↑</span><span   class="<?php if($sortOrder=='ASC'){ echo 'text-white';} ?> ">↓</span> <?php } else{ ?> <span class="" >↑</span><span   class="">↓</span> <?php } ?></span></th>
                                                ...
                                                <th onclick="displayRecordsbysort('<?php echo $limit;  ?>', '<?php echo 1; ?>','<?php echo $searchValue; ?>','<?php echo $jobstatus; ?>','RespGroup')"><b>Resp Group</b><span  style="top: 5px;right:2px;"><?php if($sortField=='RespGroup'){ ?><span class="<?php if($sortOrder=='DESC'){ echo 'text-white';} ?>" >↑</span><span   class="<?php if($sortOrder=='ASC'){ echo 'text-white';} ?> ">↓</span> <?php } else{ ?> <span class="" >↑</span><span   class="">↓</span> <?php } ?></span></th>
                                                 ...
                                                 <th onclick="displayRecordsbysort('<?php echo $limit;  ?>', '<?php echo 1; ?>','<?php echo $searchValue; ?>','<?php echo $jobstatus; ?>','DateofSubmission')"><b>Date of Submission</b><span  style="top: 5px;right:2px;"><?php if($sortField=='DateofSubmission'){ ?><span class="<?php if($sortOrder=='DESC'){ echo 'text-white';} ?>" >↑</span><span   class="<?php if($sortOrder=='ASC'){ echo 'text-white';} ?> ">↓</span> <?php } else{ ?> <span class="" >↑</span><span   class="">↓</span> <?php } ?></span></th>
                                                ...
                                                <th width="120px" onclick="displayRecordsbysort('<?php echo $limit;  ?>', '<?php echo 1; ?>','<?php echo $searchValue; ?>','<?php echo $jobstatus; ?>','Action')"><b>Action</b><span  style="top: 5px;right:2px;"><span  style="top: 5px;right:2px;"><?php if($sortField=='Action'){ ?><span class="<?php if($sortOrder=='DESC'){ echo 'text-white';} ?>" >↑</span><span   class="<?php if($sortOrder=='ASC'){ echo 'text-white';} ?> ">↓</span> <?php } else{ ?> <span class="" >↑</span><span   class="">↓</span> <?php } ?></span></th>
											...
											while($row = $result->fetch_assoc()) {
 											?>
                                                    <tr>
                                                        <td><input type="checkbox" name="reviewer[]" class="checkbox" value="<?php echo $row['order_no'];?>"></td>
                                                         ...
                                                         <td><?php if($row['mat']==''){ echo  $row['cb_mat']; } else { echo $row['mat']; } ?></td>
                                                         <?php
                                                 ...
                                                 <td><?php 
                                                  ...
                                                  if($row['complete_status']=='Complete' || $row['complete_status']=='Complete, In SAP Long Text')
                                                 {
                                                 ...
                                                 {
                                                     echo '<br><span class="job-status-color_'.$row['order_no'].'" style="color:gray;font-weight: bold;font-size: 11px;">CN-29 Eligible</span>';
                                                    
                                                 }
                                                 
                                                 if( $rowjonstatus['order_name']=='Unknown Status')
                                                 {
                                                     echo '<br><span class="job-status-color_'.$row['order_no'].'" style="color:gray;font-weight: bold;font-size: 11px;">Unknown Status</span>';
                                                 ...
                                                 {
                                                     echo '<br><span class="job-status-color_'.$row['order_no'].'" style="color:gray;font-weight: bold;font-size: 11px;">Field Remediation Required</span>';
                                                ...
                                                <td><?= date('m/d/Y',strtotime($row['dateofreview'])); ?></td>
											 ...
											 <?php }}else{
												 ?>
											<tr><td valign="top" colspan="10" class="text-center dataTables_empty">No data available in table</td></tr>
												 <?php
											 }
...
<li class="lastvalue"><span>...</span></li>
                ...
                <li class="paginate_button page-item next disabled" id="DataTables_Table_6_next lastvaluejobapp">
                 
                    <a href="javascript:void(0);" onclick="displayRecords('<?php echo $limit;  ?>', '<?php echo $total_pages; ?>','<?php echo $searchValue; ?>','<?php echo $jobstatus; ?>','<?php echo $total_pages; ?>');"  class="links"><?= $total_pages ?></a>
					</li>
...
?>
                         ...
                         <?php  }else{ ?>
                                 ...
                                 <a><button  type="button" disabled class="btn btn-primary approvediv-color mr-2 mb-2" id="submit">Approve</button></a>
                                    ...
                                    <a><button  type="button" disabled class="btn bg-success approvediv-color divcolororange mr-2 mb-2">Approve All</button></a>
									
                                <a><button  type="button" disabled class="btn bg-info approvediv-color color01 mr-2 mb-2">Approve Top 50</button></a>
...
Lobibox.confirm({
            ...
            callback: function ($this, type) {
			 ...
			 var request = $.ajax({
                                      ...
                                      type: "POST",
                                      data: {approveby:approveby,form_type:'ApproveAll'}
                                    });
...
Lobibox.confirm({
            ...
            callback: function ($this, type) {
			 ...
			 var request = $.ajax({
                                      url: "ProjectDetailsSaveJS.php",
                                      type: "POST",
                                      data: {approveby:approveby,form_type:'Approve50'}
                                    });
