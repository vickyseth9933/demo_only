(function( $ ){
    $('.editmembtn').click(function(){	
        ...
        //         return true;
        //       }else{
        //       $('#succmsg').html('Email Id Already Exists.'); 
        //       setTimeout(function() {						
        //      // location.reload();					
        //         }, 2000);
        //         return true;
      ...
      submitHandler: function(form) {
        $.ajax({
          ...
          success: function(data) {
               if(data.status=='200'){
              $('#succmsg').html('Record updated successfully.');
               $("#succmsg").removeClass("error");
               $("#succmsg").css("color", "green");
              setTimeout(function() {						
              location.reload();					
                }, 1000);
                return true;
      ...
      submitHandler: function(form) {
        $.ajax({
          ...
          success: function(data) {
              ...
              $('#successmsg').html('Email Id Already Exists.'); 
...
$('.rejectjobs').click(function(){
    $(".mform")[0].reset();
           ...
           // alert(id);
                $.ajax({				
                    ...
                    dataType: 'json',				
                    success: function(data) { 
                      //alert(data.complete_status);
                      
                     if(data.complete_status=='Complete' || data.complete_status=='Complete, In SAP Long Text') {
                         $('.show-action').hide();
                          $('.reason-reject').hide();
                ...
                var approved_date = '';    
                                  ...
                                  $("#Distribution_Transmission").val(data.distribution_transmission);
                                    ...
                                    $("#dc39_date").val(data.dc39_date);
                                     ...
                                     //$("#determine_the_MOI" ).text( data.determine_the_MOI );
 ...
 $('.viewjobs').click(function(){
    $(".mform")[0].reset();
           ...
           // alert(id);
                ...
                var approved_date = '';    
                                  ...
                                  $("#2recommendation").val(data.recommendation);
                                  ...
                                  $("#2Distribution_Transmission").val(data.distribution_transmission);
                                    ...
                                    $("#2dc39_date").val(data.dc39_date);
                                     ...
                                     if(data.CN29_in_SAP=='1'){
                                         $('#2qualfi').addClass('d-none');
                                         $( "#2CN29_in_SAP" ).prop( "checked", true );
                                     }
                                     else{
                                         $('#2qualfi').removeClass('d-none');
                                     ...
                                     if(data.order_stage=='7' && data.reject_status=='1'){
                                         $( ".2reject_cmt" ).show();
                                         $( "#2reject_cmt" ).val(data.commnets_of_reject);
                                     }else{
                                      $( ".2reject_cmt" ).hide();
                                     }
   ...
   $('.viewcompletedjobs').click(function(){
    $(".mform")[0].reset();
            //alert("edit");
            var order_no = $(this).closest('tr').find('#hiddenorderidcmpt').val();
            var id = $(this).closest('tr').find('#hiddenuseridcmpt').val();
           // $('#edituserid').val(id);	
            //alert(order_no);
           // alert(id);
                $.ajax({				
                    type: "POST",				
                    url: "ViewCompletedJobDetailsJS.php",			
                    data: {"id":id,"order_no":order_no},				
                    dataType: 'json',				
                    success: function(data) { 
                      //  alert(data.order_no);
                ...
                var approved_date = '';    
                                  ...
                                  $("#2project_id").val(data.project_id);
                                  $("#2order_no").val(order_no);
                                  $("#OREDRNO").val(order_no);
                                  $("#USERID").val(id);
                                  $("#2division").val(data.division);
                                  $("#2city").val(data.CITY);
                                  $("#2resp_gp").val(data.resp_group);
