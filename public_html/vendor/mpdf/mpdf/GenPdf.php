...
//$checklanid = $check_front_cover['reviewerlanid''];
 if($rowcount==1){
    $sql = "SELECT cb_order_new.user_id,cb_order_new.recommendation,DATE_FORMAT(cb_order_new.approved_date,'%m/%d/%Y') as approved_date,cb_order_new.order_no,cb_order_new.commnets_of_reject,cb_order_new.reject_status,cb_order_new.order_stage,cb_user.first_name,cb_user.last_name,cb_user.lanid,cb_user.city,cb_front_cover.project_id,cb_front_cover.dateofreview as created_on,cb_front_cover.reviewcompletiondate,cb_front_cover.order_description as description,cb_front_cover.resp_group,cb_front_cover.division,cb_front_cover.city as city_name,cb_front_cover.cn_29_eligible,cb_front_cover.fc_cm,cb_front_cover.ce_rcm,cb_front_cover.foreman,cb_front_cover.m_c_supervisor,cb_front_cover.distribution_transmission,cb_front_cover.inspector,
...
if($row['approved_date']=='0000-00-00 00:00:00'){
  ...
  $reviewcompletiondate = $row['approved_date']; 
...
<h5 class="font-strong mb-4 pt-4 formheadingdiv01">Cover Sheet</h5>
	...
	<tr>
		<td>City</td>
		<td class="white-bg">'.htmlentities($row['city_name'], ENT_QUOTES).'</td>
		<td>Resp Group</td>
		<td class="white-bg">'.htmlentities($row['resp_group'], ENT_QUOTES).'</td>
	</tr>
</table>
