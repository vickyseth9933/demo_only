<?php
   ...
   $monthyear = date('Y-m', strtotime($monthyear));
    ...
    if($filter=='' || $_REQUEST['jobstatusr']!='piechart'){
      $sql_Jobsfield_remediation = "SELECT count(cb_order.id) FROM `cb_order_new` INNER JOIN cb_order on cb_order.order_id = cb_order_new.order_no WHERE cb_order.status = 1 AND cb_order_new.send_job_approval='1' AND cb_order_new.order_stage= 6";
      }else{
      if($filter=='month'){    
      ...
      WHERE  cb_order.status = 1 AND DATE_FORMAT(cb_order_new.approved_date, '%Y-%m') = '$monthyear' AND cb_order_new.send_job_approval='1' AND cb_order_new.order_stage= 6";
      }
      else if($filter=='week' || $filter=='daterange'){
      $sql_Jobsfield_remediation = "SELECT count(cb_order.id) FROM `cb_order_new` 
      LEFT JOIN cb_order on cb_order.order_id = cb_order_new.order_no 
      LEFT JOIN cb_front_cover ON cb_order_new.order_no = cb_front_cover.order_id 
      WHERE cb_order.status = 1 AND DATE(cb_order_new.approved_date) >= '$sdate' AND DATE(cb_order_new.approved_date) <= '$enddate' AND cb_order_new.send_job_approval='1' AND cb_order_new.order_stage= 6";
      }
      ...
      if($filter=='' || $_REQUEST['jobstatusr']!='piechart'){
      $sql_JobsCN29 = "SELECT count(cb_order.id) FROM `cb_order_new` INNER JOIN cb_order on cb_order.order_id = cb_order_new.order_no WHERE cb_order.status = 2 AND cb_order_new.send_job_approval='1' AND cb_order_new.order_stage= 6";
      }else{
      if($filter=='month'){    
      $sql_JobsCN29 = "SELECT count(cb_order.id) FROM `cb_order_new` 
      LEFT JOIN cb_order on cb_order.order_id = cb_order_new.order_no 
      LEFT JOIN cb_front_cover ON cb_order_new.order_no = cb_front_cover.order_id 
      WHERE  cb_order.status = 2 AND DATE_FORMAT(cb_order_new.approved_date, '%Y-%m') = '$monthyear' AND cb_order_new.send_job_approval='1' AND cb_order_new.order_stage= 6";
      }
      else if($filter=='week' || $filter=='daterange'){
      $sql_JobsCN29 = "SELECT count(cb_order.id) FROM `cb_order_new` 
      LEFT JOIN cb_order on cb_order.order_id = cb_order_new.order_no 
      LEFT JOIN cb_front_cover ON cb_order_new.order_no = cb_front_cover.order_id 
      WHERE cb_order.status = 2 AND DATE(cb_order_new.approved_date) >= '$sdate' AND DATE(cb_order_new.approved_date) <= '$enddate' AND cb_order_new.send_job_approval='1' AND cb_order_new.order_stage= 6";
      }
      ...
      if($filter=='' || $_REQUEST['jobstatusr']!='piechart'){
      $sql_Jobsunknown_status = "SELECT count(cb_order.id) FROM `cb_order_new` INNER JOIN cb_order on cb_order.order_id = cb_order_new.order_no WHERE cb_order.status = 3 AND cb_order_new.send_job_approval='1' AND cb_order_new.order_stage= 6";
      }else{
      if($filter=='month'){    
      $sql_Jobsunknown_status = "SELECT count(cb_order.id) FROM `cb_order_new` 
      LEFT JOIN cb_order on cb_order.order_id = cb_order_new.order_no 
      LEFT JOIN cb_front_cover ON cb_order_new.order_no = cb_front_cover.order_id 
      WHERE  cb_order.status = 3 AND DATE_FORMAT(cb_order_new.approved_date, '%Y-%m') = '$monthyear' AND cb_order_new.send_job_approval='1' AND cb_order_new.order_stage= 6";
      }
      else if($filter=='week' || $filter=='daterange'){
      $sql_Jobsunknown_status = "SELECT count(cb_order.id) FROM `cb_order_new` 
      LEFT JOIN cb_order on cb_order.order_id = cb_order_new.order_no 
      LEFT JOIN cb_front_cover ON cb_order_new.order_no = cb_front_cover.order_id 
      WHERE cb_order.status = 3 AND DATE(cb_order_new.approved_date) >= '$sdate' AND DATE(cb_order_new.approved_date) <= '$enddate' AND cb_order_new.send_job_approval='1' AND cb_order_new.order_stage= 6";
      }
   ...
   if($_REQUEST['filter']=='' ||  $_REQUEST['jobstatusr']!='cn29_eligible'){
       $sqljob_cn29 = "SELECT cb_order_new.RESPONSIBLE_GROUP,cb_order_new.MAT as MAAT,cb_order_new.description as cb_desc,cb_order_new.TRANS_DIST,cb_order_new.approved_date,cb_order_new.order_no,cb_order_new.user_id,cb_order_new.recommendation,cb_order_new.order_stage,CONCAT(cb_user.first_name, ' ' , cb_user.last_name)  as name,cb_front_cover.order_description as description,cb_front_cover.resp_group,
   ...
   if($_REQUEST['filter']=='' || $_REQUEST['jobstatusr']!='fieldremedation'){
        $sqljob_field_remedtn = "SELECT cb_order_new.RESPONSIBLE_GROUP,cb_order_new.MAT as MAAT,cb_order_new.description as cb_desc,cb_order_new.TRANS_DIST,cb_order_new.approved_date,order_status.order_name,cb_order_new.recommendation,cb_order_new.order_no,cb_order_new.user_id,cb_order_new.order_stage,CONCAT(cb_user.first_name, ' ' , cb_user.last_name)  as name,cb_front_cover.order_description as description,cb_front_cover.resp_group,
       cb_project_details.mat FROM cb_order_new 
       INNER JOIN cb_user ON(cb_order_new.user_id=cb_user.id)
       LEFT JOIN cb_front_cover ON(cb_front_cover.order_id=cb_order_new.order_no)
       LEFT JOIN cb_project_details ON(cb_project_details.order_id=cb_order_new.order_no)
   ...
   if($_REQUEST['filter']=='' || $_REQUEST['jobstatusr']!='unknownstatus'){
       $sqljob_unknown_status = "SELECT cb_order_new.RESPONSIBLE_GROUP,cb_order_new.MAT as MAAT,cb_order_new.description as cb_desc,cb_order_new.TRANS_DIST,order_status.order_name,cb_order_new.approved_date,cb_order_new.recommendation,cb_order_new.order_no,cb_order_new.user_id,cb_order_new.order_stage,CONCAT(cb_user.first_name, ' ' , cb_user.last_name)  as name,cb_front_cover.order_description as description,cb_front_cover.resp_group,
       cb_project_details.mat FROM cb_order_new 
       INNER JOIN cb_user ON(cb_order_new.user_id=cb_user.id)
       LEFT JOIN cb_front_cover ON(cb_front_cover.order_id=cb_order_new.order_no)
       LEFT JOIN cb_project_details ON(cb_project_details.order_id=cb_order_new.order_no)
   ...
   if($_REQUEST['filter'] =='week' && $_REQUEST['jobstatusr']=='cn29_eligible'){
        $sqljob_cn29 = "SELECT cb_order_new.RESPONSIBLE_GROUP,cb_order_new.MAT as MAAT,cb_order_new.description as cb_desc,cb_order_new.TRANS_DIST,cb_order_new.approved_date,order_status.order_name,cb_order_new.order_no,cb_order_new.recommendation,cb_order_new.user_id,cb_order_new.order_stage,CONCAT(cb_user.first_name, ' ' , cb_user.last_name)  as name,cb_front_cover.order_description as description,cb_front_cover.resp_group,
                                       cb_project_details.mat FROM cb_order_new 
                                       INNER JOIN cb_user ON(cb_order_new.user_id=cb_user.id)
                                       LEFT JOIN cb_front_cover ON(cb_front_cover.order_id=cb_order_new.order_no)
                                       LEFT JOIN cb_project_details ON(cb_project_details.order_id=cb_order_new.order_no)
...
span.glyphicon.glyphicon-ok.check-mark {
  ...
  ul.nav.navbar-toolbar.pull-right {
    margin-top: 16px;
...
.custom_calendar {
    background: #fff url(https://crossfinal.epiksolution.org/assets/img/calendar-icon.png) no-repeat;
...
header.header.header a.dropdown-item .fa-lock:before {
content: "\f023";
top: 15px;
position: absolute;
left: 10px;
}
.rowdiv1 .form-control {
    border: none !important;
    box-shadow: none;
    border-radius: 2px;
}
.rowdiv1 .form-control:focus {
    box-shadow: none;
}
.close.buttondivbold {
    font-size: 25px;
                           ...
                           <div class="form-group month_year" id="month_year1">
                              ...
                              <div style="display:none" class="form-group month_year" id="month_year1">
                                 ...
                                 <div class='input-group date' id=''>
                                    <?php if($_REQUEST['filter'] =='month' && $_REQUEST['jobstatusr']=='cn29_eligible') { ?>
                                    <input class=" datepicker monthlyjob custom_calendar" name="" id="monthly1" type="text" value="<?=$_REQUEST['monthyear']?>">
                                    <?php } else { ?>
                                    <input class=" datepicker monthlyjob custom_calendar" name="" id="monthly1" type="text">
                                    <?php } ?>
                                 </div>
                              </div>
                              <?php if($_REQUEST['filter'] =='daterange' && $_REQUEST['jobstatusr']=='cn29_eligible') { ?>
                              <div class="form-group date_range" id="date_range1">
                                 ...
                                 <div style="display:none" class="form-group date_range" id="date_range1">
                                    ...
                                    <div class='input-group date' id=''>
                                       ...
                                       <input class="custom_calendar" type="text" id="from1" name="from" value="<?= $_REQUEST['sdate'] ?>">
                                       <label for="to">to</label>
                                       <input class="custom_calendar" type="text" id="to1" name="to" value="<?= $_REQUEST['enddate'] ?>">
                                       <?php }else { ?>
                                       <label for="from">From</label>
                                       <input class="custom_calendar" type="text" id="from1" name="from">
                                       <label for="to">to</label>
                                       <input class="custom_calendar" type="text" id="to1" name="to">
                                       <?php } ?>
                                    </div>
                                 </div>
                                 <?php if($_REQUEST['filter'] =='week' && $_REQUEST['jobstatusr']=='cn29_eligible') { ?>
                                 <div class="form-group weekly" id="weekly1">
                                    ...
                                    <div style="display:none" class="form-group weekly" id="weekly1">
                                       ...
                                       <div class='input-group date' id=''>
                                          <?php if($_REQUEST['filter'] =='week' && $_REQUEST['jobstatusr']=='cn29_eligible') { ?>
                                          <input class="custom_calendar datepicker3"  name="weekdays" id="weekfilter1" type="text" value="<?=$_REQUEST['sdate']. ' - '.$_REQUEST['enddate']?>">
                                          <?php } else { ?>
                                          <input class="custom_calendar datepicker3"  name="weekdays" id="weekfilter1" type="text">
                                          <?php } ?>
                                       </div>
                                    </div>
                                 </div>
                        ...
                        <div class="flexbox control-div mb-2">
                           ...
                           <div class="form-group month_year" id="month_year2">
                              ...
                              <div style="display:none" class="form-group month_year" id="month_year2">
                                 ...
                                 <div class='input-group date' id=''>
                                    <?php if($_REQUEST['filter'] =='month' && $_REQUEST['jobstatusr']=='fieldremedation') { ?>
                                    <input class="datepicker monthlyjob custom_calendar" name=""  id="monthly2" type="text"  value="<?=$_REQUEST['monthyear']?>">
                                    <?php } else { ?>
                                    <input class="datepicker monthlyjob custom_calendar" name=""  id="monthly2" type="text">
                                    <?php } ?>
                                 </div>
                              </div>
                              <?php if($_REQUEST['filter'] =='daterange' && $_REQUEST['jobstatusr']=='fieldremedation') { ?>
                              <div class="form-group date_range" id="date_range2">
                                 ...
                                 <div style="display:none" class="form-group date_range" id="date_range2">
                                    ...
                                    <div class='input-group date' id=''>
                                       <label for="from">From</label>
                                       <?php if($_REQUEST['filter'] =='daterange' && $_REQUEST['jobstatusr']=='fieldremedation') { ?>
                                       <input class="custom_calendar" type="text" id="from2" name="from" value="<?=$_REQUEST['sdate']?>">
                                       <?php } else { ?>
                                       <input class="custom_calendar" type="text" id="from2" name="from">
                                       <?php } ?>
                                       <label for="to">to</label>
                                       <?php if($_REQUEST['filter'] =='daterange' && $_REQUEST['jobstatusr']=='fieldremedation') { ?>
                                       <input class="custom_calendar" type="text" id="to2" name="to" value="<?=$_REQUEST['enddate']?>">
                                       <?php } else { ?>
                                       <input class="custom_calendar" type="text" id="to2" name="to">
                                       <?php } ?>
                                    </div>
                                 </div>
                                 <?php if($_REQUEST['filter'] =='week' && $_REQUEST['jobstatusr']=='fieldremedation') { ?>
                                 <div class="form-group weekly" id="weekly2">
                                    ...
                                    <div style="display:none" class="form-group weekly" id="weekly2">
                                       ...
                                       <div class='input-group date' id=''>
                                          <?php if($_REQUEST['filter'] =='week' && $_REQUEST['jobstatusr']=='fieldremedation') { ?>        
                                          <input class="custom_calendar datepicker3"  name="weekdays" id="weekfilter2" type="text" value="<?=$_REQUEST['sdate']. ' - '.$_REQUEST['enddate']?>">
                                          <?php } else { ?> 
                                          <input class="custom_calendar datepicker3"  name="weekdays" id="weekfilter2" type="text">
                                          <?php } ?>
                                       </div>
                                    </div>
                                 </div>
                                       ...
                                       <table class="table table-bordered table-hover" id="dash_datatable2">
                                          ...
                                          <tbody id="field_remedation">
                                             ...
                                             <tr>
                                                ...
                                                <td><?php if($rowjobstatus['description']!=''){ echo $rowjobstatus['description']; }else{ echo $rowjobstatus['cb_desc'];
                                                   }?></td>
                                                ...
                                                <td><?php if($rowjobstatus['recommendation']==null){ echo 'N/A'; } else { echo $rowjobstatus['recommendation']; }?></td>
                                                <!--<td><?php 
                                                   /*
                        ...
                        <div class="flexbox control-div mb-2">
                           ...
                           <div class="form-group month_year" id="month_year3">
                              ...
                              <div style="display:none" class="form-group month_year" id="month_year3">
                                 ...
                                 <div class='input-group date' id=''>
                                    <?php if($_REQUEST['filter'] =='month' && $_REQUEST['jobstatusr']=='unknownstatus') { ?>
                                    <input class="datepicker monthlyjob custom_calendar" name="" id="monthly3" type="text"  value="<?=$_REQUEST['monthyear']?>">
                                    <?php }  else { ?>
                                    <input class=" datepicker monthlyjob custom_calendar" name="" id="monthly3" type="text">
                                    <?php } ?> 
                                 </div>
                              </div>
                              <?php if($_REQUEST['filter'] =='daterange' && $_REQUEST['jobstatusr']=='unknownstatus') { ?>
                              <div class="form-group date_range" id="date_range3">
                                 ...
                                 <div style="display:none" class="form-group date_range" id="date_range3">
                                    ...
                                    <div class='input-group date' id=''>
                                       <label for="from">From</label>
                                       <?php if($_REQUEST['filter'] =='daterange' && $_REQUEST['jobstatusr']=='unknownstatus') { ?>
                                       <input class="custom_calendar" type="text" id="from3" name="from" value="<?=$_REQUEST['sdate']?>">
                                       <?php }  else { ?>
                                       <input class="custom_calendar" type="text" id="from3" name="from">
                                       <?php } ?> 
                                       <label for="to">to</label>
                                       <?php if($_REQUEST['filter'] =='daterange' && $_REQUEST['jobstatusr']=='unknownstatus') { ?>
                                       <input class="custom_calendar" type="text" id="to3" name="to"  value="<?=$_REQUEST['enddate']?>">
                                       <?php }  else { ?>
                                       <input class="custom_calendar" type="text" id="to3" name="to">
                                       <?php } ?> 
                                    </div>
                                 </div>
                                 <?php if($_REQUEST['filter'] =='week' && $_REQUEST['jobstatusr']=='unknownstatus') { ?>
                                 <div class="form-group weekly" id="weekly3">
                                    ...
                                    <div style="display:none" class="form-group weekly" id="weekly3">
                                       ...
                                       <div class='input-group date' id=''>
                                          <?php if($_REQUEST['filter'] =='week' && $_REQUEST['jobstatusr']=='unknownstatus') { ?>
                                          <input class="custom_calendar datepicker3"  name="weekdays" id="weekfilter3" type="text" value="<?=$_REQUEST['sdate']. ' - '.$_REQUEST['enddate']?>">
                                          <?php }  else { ?>
                                          <input class="custom_calendar datepicker3"  name="weekdays" id="weekfilter3" type="text">
                                          <?php } ?>   
                                       </div>
                                    </div>
                                 </div>
                                       ...
                                       <table class="table table-bordered table-hover" id="dash_datatable3">
                                          ...
                                          <tbody id="res_data2">
                                             ...
                                             <tr>
                                                ...
                                                <td><?php if($rownotjobstart['description']!=''){ echo $rownotjobstart['description']; }else{ echo $rownotjobstart['cb_desc'];
                                                   }?></td>
                                                ...
                                                <td><?php if($rownotjobstart['recommendation']==null){ echo 'N/A'; } else { echo $rownotjobstart['recommendation']; }?></td>
                                                <td><?php 
                        ...
                        <div class="flexbox control-div mb-2">
                           ...
                           <div class="form-group month_year" id="month_year3piechart">
                              ...
                              <div style="display:none" class="form-group month_year" id="month_year3piechart">
                                 ...
                                 <div class='input-group date' id=''>
                                    <?php if($_REQUEST['filter'] =='month' && $_REQUEST['jobstatusr']=='piechart') { ?>
                                    <input class=" datepicker monthlyjobpiechart custom_calendar" name="" id="monthly3piechart" type="text"  value="<?=$_REQUEST['monthyear']?>">
                                    <?php }  else { ?>
                                    <input class="datepicker monthlyjobpiechart custom_calendar" name="" id="monthly3piechart" type="text">
                                    <?php } ?> 
                                 </div>
                              </div>
                              <?php if($_REQUEST['filter'] =='daterange' && $_REQUEST['jobstatusr']=='piechart') { ?>
                              <div class="form-group date_range" id="date_range3piechart">
                                 ...
                                 <div style="display:none" class="form-group date_range" id="date_range3piechart">
                                    ...
                                    <div class='input-group date' id=''>
                                       <label for="from">From</label>
                                       <?php if($_REQUEST['filter'] =='daterange' && $_REQUEST['jobstatusr']=='piechart') { ?>
                                       <input class="custom_calendar" type="text" id="from3piechart" name="from" value="<?=$_REQUEST['sdate']?>">
                                       <?php }  else { ?>
                                       <input class="custom_calendar" type="text" id="from3piechart" name="from">
                                       <?php } ?> 
                                       <label for="to">to</label>
                                       <?php if($_REQUEST['filter'] =='daterange' && $_REQUEST['jobstatusr']=='piechart') { ?>
                                       <input class="custom_calendar" type="text" id="to3piechart" name="to"  value="<?=$_REQUEST['enddate']?>">
                                       <?php }  else { ?>
                                       <input class="custom_calendar" type="text" id="to3piechart" name="to">
                                       <?php } ?> 
                                    </div>
                                 </div>
                                 <?php if($_REQUEST['filter'] =='week' && $_REQUEST['jobstatusr']=='piechart') { ?>
                                 <div class="form-group weekly" id="weekly3piechart">
                                    <?php }  else { ?>
                                    <div style="display:none" class="weeklypiechart" id="weekly3piechart">
                                       <?php } ?>   
                                       <div class='input-group date' id=''>
                                          <?php if($_REQUEST['filter'] =='week' && $_REQUEST['jobstatusr']=='piechart') { ?>
                                          <input class="custom_calendar datepicker3piechart"  name="weekdays" id="weekfilter3piechart" type="text" value="<?=$_REQUEST['sdate']. ' - '.$_REQUEST['enddate']?>">
                                          <?php }  else { ?>
                                          <input class="custom_calendar datepicker3"  name="weekdays" id="weekfilter3piechart" type="text">
                                          <?php } ?>   
                                       </div>
                                    </div>
                                 </div>
