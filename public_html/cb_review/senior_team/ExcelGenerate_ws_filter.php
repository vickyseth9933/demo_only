...
if(isset($_REQUEST['type'])){
   ...
   $enddate = date('Y-m-d', strtotime($enddate1));
    ...
    if($_REQUEST['type'] == 'WSCN29-all' || $_REQUEST['type'] == 'WSCN29' || $_REQUEST['type'] == 'RespGroup-CN29'){
        
      $datatype = "CN-29 Eligible";
        
    } if($_REQUEST['type'] == 'WSRemediation-all' || $_REQUEST['type'] == 'WSRemediation' || $_REQUEST['type'] == 'RespGroup-Remidiation'){
        
       $datatype = "Field Remediation Required";
    } 
    if($_REQUEST['type'] == 'WSUnknown-all' || $_REQUEST['type'] == 'WSUnknown' || $_REQUEST['type'] == 'RespGroup-Unknown'){
        
        $datatype = "Unknown Status";
    }
    
   if($_REQUEST['data_value'] == 'TD_Operations') {
       
       $filterdata = "T&D Operations";
       
   } else if($_REQUEST['data_value'] == 'Unknown'){
       $filterdata = "Unknown";
   }else{
       $filterdata = "T&D Construction";
   }
     
    $name= $_REQUEST['type']; 
    
    if($_REQUEST['type'] == 'WSCN29-all' || $_REQUEST['type'] == 'WSRemediation-all' || $_REQUEST['type'] == 'WSUnknown-all'){
...
distribution_checklist.used_to_retrieve_the_document_cmt,distribution_checklist.Cross_Bore_Log,distribution_checklist.Cross_Bore_Log_cmt,DATE_FORMAT(cb_order_new.date_of_submission,'%m/%d/%Y') FROM cb_order_new 
        ...
        WHERE send_job_approval='1' AND order_stage= 6  AND order_status.order_name ='$datatype'";
        
    } else if($_REQUEST['type'] == 'WSCN29' || $_REQUEST['type'] == 'WSRemediation' || $_REQUEST['type'] == 'WSUnknown') {
...
distribution_checklist.used_to_retrieve_the_document_cmt,distribution_checklist.Cross_Bore_Log,distribution_checklist.Cross_Bore_Log_cmt,DATE_FORMAT(cb_order_new.date_of_submission,'%m/%d/%Y') FROM cb_order_new 
        ...
        LEFT JOIN qualiflying_five ON(qualiflying_five.order_id=cb_order_new.order_no)
        LEFT JOIN distribution_checklist ON(distribution_checklist.order_id=cb_order_new.order_no)
        WHERE send_job_approval='1' AND order_stage= 6  AND order_status.order_name ='$datatype' AND cb_order_new.approved_date >= '$today' AND cb_order_new.approved_date <= '$enddate'";
...
distribution_checklist.used_to_retrieve_the_document_cmt,distribution_checklist.Cross_Bore_Log,distribution_checklist.Cross_Bore_Log_cmt,DATE_FORMAT(cb_order_new.date_of_submission,'%m/%d/%Y') FROM cb_order_new 
        ...
        LEFT JOIN qualiflying_five ON(qualiflying_five.order_id=cb_order_new.order_no)
        LEFT JOIN distribution_checklist ON(distribution_checklist.order_id=cb_order_new.order_no)
        WHERE send_job_approval='1' AND order_stage= 6  AND order_status.order_name ='$datatype' AND cb_front_cover.resp_group = '$filterdata'";  
    }
