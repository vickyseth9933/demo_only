...
//for mat count
$c_que = "select count(cb_project_details.mat) as mat_count from cb_project_details 
...
INNER JOIN order_status ON(cb_order.status =order_status.id) where cb_order_new.send_job_approval='1' AND cb_project_details.mat != ''";
...
//for mat listing
$show_que = "SELECT cb_order_new.CITY,order_status.order_name,cb_order_new.order_no,cb_order_new.user_id,cb_order_new.order_stage,CONCAT(cb_user.first_name, ' ' , cb_user.last_name)  as name,cb_front_cover.order_description as description,cb_front_cover.resp_group,
...
WHERE cb_order_new.send_job_approval='1' AND cb_project_details.mat != ''";
...
$resp_que = "select count(cb_front_cover.resp_group) as resp_grp from cb_front_cover 
INNER JOIN cb_project_details ON(cb_front_cover.order_id = cb_project_details.order_id)
INNER JOIN cb_order_new ON(cb_order_new.order_no = cb_project_details.order_id)
INNER JOIN cb_order ON(cb_order.order_id = cb_project_details.order_id)
INNER JOIN order_status ON(cb_order.status =order_status.id) where cb_order_new.send_job_approval='1' AND cb_front_cover.resp_group != ''";
$resp_res = $conn->query($resp_que);
$resp_row = $resp_res->fetch_array();
print_r($resp_row);
//echo $crow;

//for resp group listing

//for mat listing
$respshow_que = "SELECT cb_order_new.CITY,order_status.order_name,cb_order_new.order_no,cb_order_new.user_id,cb_order_new.order_stage,CONCAT(cb_user.first_name, ' ' , cb_user.last_name)  as name,cb_front_cover.order_description as description,cb_front_cover.resp_group,
cb_project_details.mat FROM cb_order_new 
INNER JOIN cb_user ON(cb_order_new.user_id=cb_user.id)
INNER JOIN cb_front_cover ON(cb_front_cover.order_id=cb_order_new.order_no)
INNER JOIN cb_project_details ON(cb_project_details.order_id=cb_order_new.order_no)
INNER JOIN cb_order ON(cb_order.order_id=cb_order_new.order_no)
INNER JOIN  order_status ON(cb_order.status=order_status.id)
WHERE cb_order_new.send_job_approval='1' AND cb_front_cover.resp_group != ''";

$respshow_res = $conn->query($respshow_que);
while($respshow_row = $respshow_res->fetch_array()) {
//print_r($respshow_row);
}
/*******************Queries End************************/ 
...
<li><a href="active-jobs.html" title="Active Jobs">(1) Active Jobs</a></li>
<li><a href="#" title="View Jobs By NDE Provider">(2) View Jobs By NDE Provider</a></li>
<li><a href="view-job-city.html" title="View Jobs By City">(3) View Jobs By City</a></li>
<li><a href="#" title="View Jobs By Pipe Size">(4) View Jobs By Pipe Size</a></li>
<li><a href="#" title="View Jobs By Work Stream">(5) View Jobs By Work Stream</a></li>
<li><a href="completed-job.html" title="Completed Jobs">(6) Completed Jobs</a></li>
</ul>
</div>
                           </div>
                        </div>
