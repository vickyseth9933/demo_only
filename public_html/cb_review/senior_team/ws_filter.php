...
} else if($_REQUEST['send_value'] == 'Unknown'){
     $filterval = "UNKNOWN"; 
}else{
...
WHERE order_status.order_name = 'CN-29 Eligible' AND  send_job_approval='1' AND order_stage= 6 AND cb_front_cover.resp_group = '$filterval'"; 
...
LEFT JOIN cb_order ON(cb_order.order_id=cb_order_new.order_no)
LEFT JOIN  order_status ON(cb_order.status=order_status.id)
WHERE order_status.order_name = 'CN-29 Eligible' AND  send_job_approval='1' AND order_stage= 6 AND cb_order_new.approved_date >= '$today' AND cb_order_new.approved_date <= '$enddate'";   
}
else {
...
LEFT JOIN cb_order ON(cb_order.order_id=cb_order_new.order_no)
LEFT JOIN  order_status ON(cb_order.status=order_status.id)
WHERE order_status.order_name = 'Field Remediation Required' AND  send_job_approval='1' AND order_stage= 6 AND cb_front_cover.resp_group = '$filterval'";  
}
...
LEFT JOIN cb_order ON(cb_order.order_id=cb_order_new.order_no)
LEFT JOIN  order_status ON(cb_order.status=order_status.id)
WHERE order_status.order_name = 'Field Remediation Required' AND  send_job_approval='1' AND order_stage= 6 AND cb_order_new.approved_date >= '$today' AND cb_order_new.approved_date <= '$enddate'";  
...
$sqljob_unknown_status = "SELECT cb_order_new.RESPONSIBLE_GROUP,cb_order_new.MAT as MAAT,cb_order_new.description as cb_desc,cb_order_new.TRANS_DIST,cb_order_new.approved_date,cb_order_new.recommendation,cb_order_new.total_dollars,cb_order_new.order_no,cb_order_new.user_id,cb_order_new.order_stage,CONCAT(cb_user.first_name, ' ' , cb_user.last_name)  as name,cb_front_cover.order_description as description,cb_front_cover.resp_group,
cb_project_details.mat FROM cb_order_new 
INNER JOIN cb_user ON(cb_order_new.user_id=cb_user.id)
LEFT JOIN cb_front_cover ON(cb_front_cover.order_id=cb_order_new.order_no)
LEFT JOIN cb_project_details ON(cb_project_details.order_id=cb_order_new.order_no)
LEFT JOIN cb_order ON(cb_order.order_id=cb_order_new.order_no)
LEFT JOIN  order_status ON(cb_order.status=order_status.id)
WHERE order_status.order_name = 'Unknown Status' AND  send_job_approval='1' AND order_stage= 6 AND cb_front_cover.resp_group = '$filterval'"; 
}
...
LEFT JOIN cb_order ON(cb_order.order_id=cb_order_new.order_no)
LEFT JOIN  order_status ON(cb_order.status=order_status.id)
WHERE order_status.order_name = 'Unknown Status' AND  send_job_approval='1' AND order_stage= 6 AND cb_order_new.approved_date >= '$today' AND cb_order_new.approved_date <= '$enddate'";  
} 

else {
    
$sqljob_unknown_status = "SELECT cb_order_new.RESPONSIBLE_GROUP,cb_order_new.MAT as MAAT,cb_order_new.description as cb_desc,cb_order_new.TRANS_DIST,cb_order_new.recommendation,cb_order_new.approved_date,cb_order_new.total_dollars,cb_order_new.order_no,cb_order_new.user_id,cb_order_new.order_stage,CONCAT(cb_user.first_name, ' ' , cb_user.last_name)  as name,cb_front_cover.order_description as description,cb_front_cover.resp_group,
cb_project_details.mat FROM cb_order_new 
INNER JOIN cb_user ON(cb_order_new.user_id=cb_user.id)
LEFT JOIN cb_front_cover ON(cb_front_cover.order_id=cb_order_new.order_no)
LEFT JOIN cb_project_details ON(cb_project_details.order_id=cb_order_new.order_no)
...
<script>
  $( function() {
    ...
    var type='CN29';
    window.location.href="ws_filter.php?date="+startDat+"&enddate="+Enddate+"&type="+type;
    }
    });
    });
...
.w15 {
                            ...
                            <div class="ibox-body">
                                ...
                                <div class="d-flex justify-content-between flexible-status-search">
                                    	...
                                    	<div class="flexbox control-div mb-2">
                                            ...
                                            <option <?php  if($_REQUEST['send_value'] == 'TD_Construction' && $_REQUEST['type'] == 'CN29') { echo "selected"; } ?> value="TD_Construction">T&D Construction</option>
                                            ...
                                            <option <?php if($_REQUEST['send_value'] == 'Unknown' && $_REQUEST['type'] == 'CN29') { echo "selected"; } ?> value="Unknown">Unknown</option>
										...
										<div class="flexbox control-div mb-2">
											<div class="flexbox">
												<label class="mb-0 pr-2 ml-0 w15">Week Filter:</label>
												<input type="text" class="selectpicker show-tick form-control selectpickerdiv2" onChange="datefun(datepicker.value,'CN29')" id="datepicker" title="Please select date" data-style="btn-solid" data-width="150px" value="<?php if($type == 'CN29') { echo $data. ' - ' .$enddate1; }  ?>">
											</div>
										</div>
                                ...
                                <div class="row mb-4">
                                                    ...
                                                    <tr>
                                                        <!--<td><input type="checkbox" name="reviewer[]" class="checkbox" value="<?php echo $row['order_no'];?>"></td>-->
                                                         ...
                                                         <input type="hidden" name="memid" value="<?php echo $row['user_id'];?>" id="hiddenuserid">
                                                            ...
                                                            <td><?php if($row['approved_date'] != '' && $row['approved_date'] > 0) { echo date('m/d/Y', strtotime($row['approved_date'])); } else {echo "N/A"; } ?></td>
                                 ...
                                 <div class="row">
                                        <div class="col-sm-12">
                                             <?php 
                                             if($_REQUEST['send_value'] == 'TD_Operations' || $_REQUEST['send_value'] == 'TD_Construction' || $_REQUEST['send_value'] == 'Unknown'){
                                                 $resgrp = 1;
                                             }
                                             if($resgrp == 1 && $_REQUEST['type'] == 'CN29'){
                                             ...
                                             <a href="../senior_team/ExcelGenerate_ws_filter.php?type=RespGroup-CN29&data_value=<?php echo $_REQUEST['send_value']; ?>"  class="btn btn-primary approvediv-colorr">Download Resp Group</a>  
                                              ...
                                              <a href="../senior_team/ExcelGenerate_ws_filter.php?type=WSCN29&date_value=<?php  echo $data;   ?>&enddate=<?php  echo $enddate;   ?>"  class="btn btn-primary approvediv-colorr">Download Weekly Excel</a>
                    ...
                    <div class="col-sm-12">
                        <div class="ibox">
                            <div class="ibox-body">
                                <div class="row">
									<div class="col-sm-12">
										<h5 class="font-strong mb-4">Field Remediation Required Jobs</h5>
									</div>
                                </div>
                                <div class="d-flex justify-content-between flexible-status-search">
                                    	...
                                    	<div class="flexbox control-div mb-2">
                                            ...
                                            <option <?php if($_REQUEST['send_value'] == 'Unknown' && $_REQUEST['type'] == 'Remediation') { echo "selected"; } ?> value="Unknown">Unknown</option>
                                    	...
                                    	<div class="flexbox control-div mb-2">
											<div class="flexbox">
												<label class="mb-0 pr-2 ml-0 w15">Week Filter:</label>
												<input type="text" class="selectpicker show-tick form-control selectpickerdiv2" onChange="datefun(datepicker2.value,'Remediation')" id="datepicker2" title="Please select date" data-style="btn-solid" data-width="150px" value="<?php if($type == 'Remediation') { echo $data. ' - ' .$enddate1; }  ?>">
											</div>
										</div>
										...
										<?php
											...
											<tr>
                                                ...
                                                <td><?php if($rowjobstatus['description']!=''){ echo $rowjobstatus['description']; }else{ echo $rowjobstatus['cb_desc'];
                                                 ...
                                                 <td><?php if($rowjobstatus['approved_date'] != '' && $rowjobstatus['approved_date'] > 0) { echo date('m/d/Y', strtotime($rowjobstatus['approved_date'])); } else {echo "N/A"; } ?></td>
 ...
 <div class="row">
     ...
     <div class="col-sm-12">
                                             ...
                                             if($resgrp == 1 && $_REQUEST['type'] == 'Remediation'){
                                             //if($_REQUEST['send_value'] != ''  && $_REQUEST['type'] == 'CN29') { ?>
                                             ...
                                             <a href="../senior_team/ExcelGenerate_ws_filter.php?type=RespGroup-Remidiation&data_value=<?php echo $_REQUEST['send_value']; ?>"  class="btn btn-primary approvediv-colorr">Download Resp Group</a>  
                                              ...
                                              <a href="../senior_team/ExcelGenerate_ws_filter.php?type=WSRemediation&date_value=<?php  echo $data;   ?>&enddate=<?php  echo $enddate;   ?>"  class="btn btn-primary approvediv-colorr">Download Weekly Excel</a>
                    ...
                    <div class="col-sm-12">
                        <div class="ibox">
                            <div class="ibox-body">
                                <div class="row">
									<div class="col-sm-12">
										<h5 class="font-strong mb-4">Unknown Status Jobs</h5>
									</div>
                                </div>
                                <div class="d-flex justify-content-between flexible-status-search">
                                    	...
                                    	<div class="flexbox control-div mb-2">
                                            ...
                                            <option <?php if($_REQUEST['send_value'] == 'Unknown' && $_REQUEST['type'] == 'Unknown') { echo "selected"; } ?> value="Unknown">Unknown</option>
                                    	...
                                    	<div class="flexbox control-div mb-2">
											<div class="flexbox">
												<label class="mb-0 pr-2 ml-0 w15">Week Filter:</label>
												<input type="text" class="selectpicker show-tick form-control selectpickerdiv2" onChange="datefun(datepicker3.value,'Unknown')" id="datepicker3" title="Please select date" data-style="btn-solid" data-width="150px" value="<?php if($type == 'Unknown') { echo $data. ' - ' .$enddate1; }  ?>">
											</div>
										</div>
                                ...
                                <div class="row mb-4">
										...
										<?php
											...
											<tr>
                                                ...
                                                <td><?php if($rownotjobstart['description']!=''){ echo $rownotjobstart['description']; }else{ echo $rownotjobstart['cb_desc'];
                                                 ...
                                                 <td><?php if($rownotjobstart['approved_date'] != '' && $rownotjobstart['approved_date'] > 0) { echo date('m/d/Y', strtotime($rownotjobstart['approved_date'])); } else {echo "N/A"; } ?></td>
                                 ...
                                 <div class="row">
                                        <div class="col-sm-12">
                                             ...
                                             if($resgrp == 1 && $_REQUEST['type'] == 'Unknown'){
                                             //if($_REQUEST['send_value'] != ''  && $_REQUEST['type'] == 'CN29') { ?>
                                             ...
                                             <a href="../senior_team/ExcelGenerate_ws_filter.php?type=RespGroup-Unknown&data_value=<?php echo $_REQUEST['send_value']; ?>"  class="btn btn-primary approvediv-colorr">Download Resp Group</a>  
                                              ...
                                              <?php } else if($_REQUEST['send_value'] != 'TD_Operations'  && $_REQUEST['type'] == 'Unknown') { ?>
                                                ...
                                                <a href="../senior_team/ExcelGenerate_ws_filter.php?type=WSUnknown&date_value=<?php  echo $data;  ?>&enddate=<?php  echo $enddate;   ?>"  class="btn btn-primary approvediv-colorr">Download Weekly Excel</a>
         ...
         <script src="<?= $baseurl ?>/cb_review/crossbore_main.js"></script>
...
{
   // alert (data);
    window.location.href="ws_filter.php?send_value="+data+"&type="+job_type;
}
</script>
  ...
  function datefun(date,type){
     // alert(date);
      //  window.location.href="ws_filter.php?date="+date+"&type="+type;
  }
  </script>
 <script>
