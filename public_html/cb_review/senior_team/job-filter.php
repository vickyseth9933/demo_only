...
$sqljob_data = "SELECT cb_order_new.RESPONSIBLE_GROUP,cb_order_new.MAT as MAAT,cb_order_new.description as cb_desc,cb_order_new.TRANS_DIST,IFNULL(cb_front_cover.city,cb_order_new.CITY) as CITY,order_status.order_name,cb_order_new.order_no,cb_order_new.recommendation,cb_order_new.user_id,cb_order_new.order_stage,CONCAT(cb_user.first_name, ' ' , cb_user.last_name)  as name,cb_front_cover.order_description as description,cb_front_cover.resp_group,
...
WHERE send_job_approval='1' AND order_stage= 6 AND cb_order_new.CN24_SAP_DATE !=''"; 
 ...
 $sqljob_data = "SELECT cb_order_new.RESPONSIBLE_GROUP,cb_order_new.MAT as MAAT,cb_order_new.description as cb_desc,cb_order_new.TRANS_DIST,IFNULL(cb_front_cover.city,cb_order_new.CITY) as CITY,order_status.order_name,cb_order_new.order_no,cb_order_new.recommendation,cb_order_new.user_id,cb_order_new.order_stage,CONCAT(cb_user.first_name, ' ' , cb_user.last_name)  as name,cb_front_cover.order_description as description,cb_front_cover.resp_group,
cb_project_details.mat,cb_order_new.CN24_SAP_DATE as job_copleted_date,cb_front_cover.fc_cm as j_cm,cb_front_cover.ce_rcm as j_rcm,cb_front_cover.foreman as j_forman,cb_user.lanid as j_landid,cb_front_cover.m_c_supervisor as j_supervisior 
 FROM cb_order_new 
INNER JOIN cb_user ON(cb_order_new.user_id=cb_user.id)
LEFT JOIN cb_front_cover ON(cb_front_cover.order_id=cb_order_new.order_no)
LEFT JOIN cb_project_details ON(cb_project_details.order_id=cb_order_new.order_no)
...
$sqljob_data = "SELECT cb_order_new.RESPONSIBLE_GROUP,cb_order_new.MAT as MAAT,cb_order_new.description as cb_desc,cb_order_new.TRANS_DIST,IFNULL(cb_front_cover.city,cb_order_new.CITY) as CITY,order_status.order_name,cb_order_new.order_no,cb_order_new.recommendation,cb_order_new.user_id,cb_order_new.order_stage,CONCAT(cb_user.first_name, ' ' , cb_user.last_name) as name,
...
distribution_checklist.MOI_for_Main,distribution_checklist.determine_the_MOI,distribution_checklist.SAP,distribution_checklist.PRE_Inspection,distribution_checklist.PRE_Inspection_cmt,
distribution_checklist.POST_Inspection,distribution_checklist.POST_Inspection_cmt,distribution_checklist.Cross_Bore_Log,distribution_checklist.Cross_Bore_Log_cmt
FROM cb_order_new INNER JOIN cb_user ON(cb_order_new.user_id=cb_user.id) 
INNER JOIN cb_front_cover ON(cb_front_cover.order_id=cb_order_new.order_no) 
LEFT JOIN cb_project_details ON(cb_project_details.order_id=cb_order_new.order_no) 
LEFT JOIN cb_order ON(cb_order.order_id=cb_order_new.order_no) 
LEFT JOIN order_status ON(cb_order.status=order_status.id) INNER JOIN distribution_checklist ON (cb_front_cover.order_id=distribution_checklist.order_id) 
                            ...
                            <div class="ibox-body">
                                ...
                                <div class="row mb-4">
                                    <div class="col-sm-12">
                                        ...
                                        <!--distribution_checklist.PRE_Inspection_cmt,
distribution_checklist.POST_Inspection,distribution_checklist.POST_Inspection_cmt,distribution_checklist.Cross_Bore_Log,distribution_checklist.Cross_Bore_Log_cmt-->
                                            ...
                                            <table class="table table-bordered table-hover" id="dash_datatablesnr">
                                                <thead class="thead-default thead-lg">
                                                    <tr>
                                                        ...
                                                        <th>Pre Inspection Cmt</th>
                                                        <th>Post Inspection</th>
                                                        <th>Post Inspection Cmt</th>
                                                        <?php } else { ?>
...
distribution_checklist.POST_Inspection,distribution_checklist.POST_Inspection_cmt,distribution_checklist.Cross_Bore_Log,distribution_checklist.Cross_Bore_Log_cmt-->
                                                      ...
                                                      <td><?php if($row['PRE_Inspection_cmt'] != ''){echo $row['PRE_Inspection_cmt'];} else{echo "N/A";}  ?></td>
                                                      <td><?php if($row['POST_Inspection'] != ''){echo $row['POST_Inspection'];} else{echo "N/A";}  ?></td>
                                                      <td><?php if($row['POST_Inspection_cmt'] != ''){echo $row['POST_Inspection_cmt'];} else{echo "N/A";}  ?></td>
                                                    <?php } else { ?>
                                                   ...
                                                   ?> 
                                                      <td><?php if($row['j_cm'] != ''){echo $row['j_cm'];}else{echo "N/A";}  ?></td>
                                                      <td><?php if($row['j_rcm'] !=''){ echo $row['j_rcm'];}else{echo "N/A";} ?></td>
                                                      <td><?php if($row['j_forman'] != ''){ echo $row['j_forman'];} else {echo "N/A";} ?></td>
                                                      <td><?php if($row['j_supervisior'] != ''){echo $row['j_supervisior'];} else{echo "N/A";} ?></td>
                                                      <td><?php if($row['j_landid'] != ''){ echo $row['j_landid'];}else{echo "N/A";} ?></td>
...
<script>
    ...
    $('#type-filtersnr').on('change', function() {
         var send_value = '<?php echo $_GET['send_value'] ?>';
     //   alert(send_value);
        if(send_value==''){
        table.column(6).search($(this).val()).draw();
    }else if(send_value=='City_Division'){
    table.column(7).search($(this).val()).draw();    
    }
    else if(send_value=='CN24Comp_date'){
    table.column(7).search($(this).val()).draw();    
    }
    else if(send_value=='roleWise'){
